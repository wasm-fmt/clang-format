...
static cl::list<std::string> LineRanges("lines",
                                        cl::desc("<start line>:<end line> - format a range of\n"
                                                 ...
                                                 "Can only be used with one input file."),
                                        cl::cat(ClangFormatCategory));
static cl::opt<std::string> Style("style",
                                  cl::desc(clang::format::StyleOptionHelpDescription),
                                  cl::init(clang::format::DefaultFormatStyle),
                                  cl::cat(ClangFormatCategory));
static cl::opt<std::string> FallbackStyle("fallback-style",
...
static cl::opt<bool> DumpConfig("dump-config",
                                ...
                                cl::cat(ClangFormatCategory));
static cl::opt<unsigned> Cursor("cursor",
...
static cl::opt<std::string> QualifierAlignment("qualifier-alignment",
...
static cl::alias DryRunShort("n",
                             cl::desc("Alias for --dry-run"),
                             cl::cat(ClangFormatCategory),
                             cl::aliasopt(DryRun),
                             cl::NotHidden);

// Emulate being able to turn on/off the warning.
static cl::opt<bool> WarnFormat("Wclang-format-violations",
...
static cl::opt<bool> NoWarnFormat("Wno-clang-format-violations",
...
static cl::opt<unsigned> ErrorLimit("ferror-limit",
...
static cl::bits<WNoError> WNoErrorList("Wno-error",
                                       cl::desc("If set don't error out on the specified warning type."),
                                       cl::values(clEnumValN(WNoError::Unknown,
                                                             "unknown",
...
static cl::opt<bool> ShowColors("fcolor-diagnostics",
...
static cl::opt<bool> NoShowColors("fno-color-diagnostics",
                                  cl::desc("If set, and on a color-capable terminal controls "
                                           "whether or not to print diagnostics in color"),
                                  cl::init(false),
                                  cl::cat(ClangFormatCategory),
...
static cl::list<std::string> FileNames(cl::Positional, cl::desc("[<file> ...]"), cl::cat(ClangFormatCategory));
...
static bool parseLineRange(StringRef Input, unsigned& FromLine, unsigned& ToLine) {
    std::pair<StringRef, StringRef> LineRange = Input.split(':');
    return LineRange.first.getAsInteger(0, FromLine) || LineRange.second.getAsInteger(0, ToLine);
}

static bool fillRanges(MemoryBuffer* Code, std::vector<tooling::Range>& Ranges) {
    ...
    for (unsigned i = 0, e = Offsets.size(); i != e; ++i) {
        ...
        if (i < Lengths.size()) {
            if (Offsets[i] + Lengths[i] > Code->getBufferSize()) {
                errs() << "error: invalid length " << Lengths[i] << ", offset + length (" << Offsets[i] + Lengths[i]
                       << ") is outside the file.\n";
                return true;
            }
...
static bool emitReplacementWarnings(const Replacements& Replaces,
    ...
    if (WarnFormat && !NoWarnFormat) {
        ...
        Mgr.AddNewSourceBuffer(MemoryBuffer::getMemBuffer(StartBuf, AssumedFileName), SMLoc());
        for (const auto& R : Replaces) {
            SMDiagnostic Diag =
                Mgr.GetMessage(SMLoc::getFromPointer(StartBuf + R.getOffset()),
                               WarningsAsErrors ? SourceMgr::DiagKind::DK_Error : SourceMgr::DiagKind::DK_Warning,
...
static void outputXML(const Replacements& Replaces,
    ...
    if (Cursor.getNumOccurrences() != 0) {
        outs() << "<cursor>" << FormatChanges.getShiftedCodePosition(CursorPosition) << "</cursor>\n";
    }
...
static bool format(StringRef FileName) {
    ...
    ErrorOr<std::unique_ptr<MemoryBuffer>> CodeOrErr =
        !OutputXML && Inplace ? MemoryBuffer::getFileAsStream(FileName) : MemoryBuffer::getFileOrSTDIN(FileName);
    ...
    llvm::Expected<FormatStyle> FormatStyle = getStyle(Style, AssumedFileName, FallbackStyle, Code->getBuffer(),
...
static int dumpConfig() {
    ...
    if (FileNames.empty()) {
        ...
        // detect the language.
        ErrorOr<std::unique_ptr<MemoryBuffer>> CodeOrErr = MemoryBuffer::getFileOrSTDIN(FileNames[0]);
        if (std::error_code EC = CodeOrErr.getError()) {
    ...
    llvm::Expected<clang::format::FormatStyle> FormatStyle =
        clang::format::getStyle(Style, FileName, FallbackStyle, Code ? Code->getBuffer() : "");
    if (!FormatStyle) {
        llvm::errs() << llvm::toString(FormatStyle.takeError()) << "\n";
        return 1;
    }
