...
install and activate Emscripten manually, install `wasi-sdk` and `wasm-tools`
manually, and either set `WASI_SDK_PATH` to the directory containing
`share/cmake/wasi-sdk-p1.cmake` or put `wasm32-wasip1-clang` on `PATH`. The
build script consumes the prepared environment; it does not invoke mise itself.
